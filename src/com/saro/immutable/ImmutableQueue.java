...
/**
 * An ImmutableQueue concrete class representation
 * 
 * @author Saravanan
 *
 * @param <T>generic type for the element of queue
 */
public final class ImmutableQueue<T> implements Queue<T>{
	...
	public final static Stack reverseStack(Stack stack) throws Exception{
		...
		Stack reverse = ImmutableStack.getEmptyStack();
		
		while(!stack.isEmpty()){
			reverse = reverse.push(stack.head());
			stack =stack.pop();
		}
